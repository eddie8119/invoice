...
export const contractApi = {
  ...
  getContract: async (id: string): Promise<ApiResponse<CompanyDTO>> => {
    return axiosInstance.get(`/contracts/${id}`);
  },
  updateContract: async (data: any): Promise<ApiResponse<CompanyDTO>> => {
    return axiosInstance.put('/contracts', data);
  },
  createContract: async (data: any): Promise<ApiResponse<CompanyDTO>> => {
